...
import { AuthContext, AuthData, localStorageKeys, Role } from '../AuthContextType';
...
const Page: FC = () => {
  ...
  if (
    ...
    authDataFromStorage = {
      ...
      roles: localStorage.getItem(localStorageKeys.roles)!.split(',') as Role[],
      Authorization: localStorage.getItem(localStorageKeys.Authorization)!,
    };
  }
  ...
  const authDataUpdate = useMemo(
    () => (data: AuthData | null) => {
      if (data) {
        ...
        window.localStorage.setItem(localStorageKeys.password, data.password);
        window.localStorage.setItem(localStorageKeys.Authorization, data.Authorization);
        window.localStorage.setItem(localStorageKeys.roles, data.roles.join(','));
      } else {
    ...
    <AuthContext.Provider value={authContextValue}>
      ...
      >
        <AppShell.Header style={{ boxShadow: '0 0 30px 0 rgba(0, 0, 0, 0.15)' }}>
          <Group flex={1} maw={900} mx="auto" style={{ justifyContent: 'center' }} h="100%">
            ...
            <Group flex={1} style={{ justifyContent: 'space-between' }} px={10}>
              ...
              <Flex justify="space-between" px="md" gap={40} visibleFrom="sm">
                <Button fw={500} variant="subtle" component={Link} to={`${settings.browserBaseURL}/recipes`}>
                  Recipes
                </Button>

                {authData && (
                  <Button fw={500} variant="subtle" component={Link} to={`${settings.browserBaseURL}/my-profile`}>
                    My profile
                  </Button>
                )}
                {!authData && (
                  <Button fw={500} variant="subtle" component={Link} to={`${settings.browserBaseURL}/sign-in`}>
                    Sign in
                  </Button>
                )}
              </Flex>
        ...
        <AppShell.Footer style={{ boxShadow: '0 0 10px 0 rgba(0, 0, 0, 0.15)' }}>
